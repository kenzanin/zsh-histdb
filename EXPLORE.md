...
├── libsql-history-skim.zsh # skim ZLE widgets
...
| Group by argv | `GROUP BY argv ORDER BY MAX(start_time) DESC` | Up/Down, skim |
| Current host first | `ORDER BY MAX(CASE WHEN host='X'...` | Up/Down |
| Current dir first | `ORDER BY CASE WHEN dir LIKE 'PWD%'...` | skim |
...
- **Ctrl+R** — `histdb-skim` (fuzzy interactive history search)
