...
## Overview
Fork of [larkery/zsh-histdb](https://github.com/larkery/zsh-histdb) by Tom Hinton, modified to use **rqlite** (distributed SQLite via HTTP API) instead of local SQLite file. Stores shell history with metadata: timestamps, working directory, hostname, session ID, exit status, duration.
...
├── rqlite-history.zsh                  # Core engine (always loaded)
├── rqlite-history-fzf.zsh              # fzf ZLE widgets
...
│   ├── histdb-merge                   # Merge from another rqlite node
...
### Core Engine (`rqlite-history.zsh`)
- **`_histdb_query`** — Central query dispatcher. Routes SQL via HTTP API (curl+jq) or rlite C client (auto-detected).
...
- **`_histdb-up-line-or-beginning-search`** — ZLE widget for Up arrow. Queries rqlite for commands matching buffer prefix (or all commands if buffer empty). Caches results for cycling.
...
Changed the gate from "is the query string non-empty?" to **"do we have cached results?"**. Now empty-buffer Up cycling works: second Up press advances to the next cached result instead of re-querying with the buffer as a prefix.

## Database (rqlite)
- **URL:** `http://127.1.1.1:50001` (default)
- **Daemon:** `rqlited` running via systemd user service `zshdb.service`
- **Data dir:** `~/.local/share/zshdb_data`
- **Stats:** 1378 commands, 1758 history entries, 123 places (as of 2026-06-03)
- **Leader:** Single node at `127.1.1.1:50002`
- **Extensions:** 14 SQLite extensions loaded (regexp, stats, fuzzy, uuid, etc.)
...
## Shell Hooks
- `zshaddhistory` → `_histdb_addhistory` (save command to rqlite)
...
- All queries go through rqlite HTTP API — no local SQLite file access
