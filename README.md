...
- **04/05/26**: Added fuzzy history search with `histdb-fzf` function (requires fzf). Supports Enter to select commands and Ctrl-J to jump to the command's directory. Added systemd user service setup for rqlite.
...
`histdb-fzf` — fuzzy finder over sqld history database. Replaces traditional reverse-isearch.

### Setup

```zsh
bindkey '^R' histdb-fzf   # Ctrl+R for fuzzy search
# or
bindkey '^[r' histdb-fzf  # Alt+R to keep default Ctrl+R
```

### Usage

- **Enter** — Insert command into buffer
- **Ctrl-J** — `cd` to command's directory
- **Ctrl-K** — Delete entry from history
- **Ctrl-C** — Cancel

Shows: command with preview, host, directory, timestamp.
